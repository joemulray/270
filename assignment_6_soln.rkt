...
Finally, add an ASCII text file "comments.txt" into the same directory where you put assignment_7.rkt, with the following contents:
...
#f)
(check-equal? 
(bool-eval '(iff b a) '( (a #t) (b #f))) ;True
#t)
(check-equal? 
...
(define (get-variables expression)
 (cond
  ;Case 1: The list is empty, nothing to do
  [ (null? expression) null ]
  ;Case 2: The input is a variable like a,b
  ;Use cons to make a list to return
  [ (is-variable? expression) (cons expression null) ]
  ;Case 3:If the expression is not a list, then it is a reserved work like or, and, not
  [ (not (list? expression)) null ]
  ;Case 4:
  [
  ;If the expression is a list, then we need to break it up 
  ;4.) Remove the duplicates
  (remove-duplicates
   ;3.) Append together our two partial lists
   (append
    ;1.) Get any variables in the first part of the expression
    (get-variables (first expression))
    ;2.) Get any variables in the rest of the expression
    (get-variables (rest expression))
  );end append
...
(define (insert_binding binding environments)
  ...
  ;Base case, no environments
  (if (null? environments)
    ;then nothing to do
    null
    ;else insert 1 binding and repeat
    (cons
     (append binding (first environments))
     (insert_binding binding (rest environments))
    );end of cons
  );end of if
);end of defun
...
;   ( (b nil) ) ;Second row has one column
; ) ; end of table
...
(define (insert_multiple_bindings bindings environments)
 ;This time we are going through the bindings
 ;We need 2 base cases
 (cond
  [ (null? environments) bindings ]
  [ (null? bindings) null ]
  [ else ;Recursive case /**** Check this
   (append
   (insert_binding (first bindings) environments)
   (insert_multiple_bindings (rest bindings) environments)
   );end of append
  ];end rescursive case
 );end of cond
)
...
(define (extend_table var_name current_table)
  ;We already know how to insert the bindings
  ;We just need to get the bindings for the var_name we get as input
...
(define (make-truth-table var_names)
  ;Base Case, no names means an empty table
  (if
    (null? var_names)
    ;then
    null
    ;else
    ;Extend the table by 1 variable name
...
(check-equal? 
(make-truth-table '(a));returns ( ((a t)) ((a nil)))
'( ((a #t)) ((a #f))))
...
(define (run-on-truth-table expression tt)
 ;Base Case, there is nothing left in the truth table
  (if (null? tt) 
    ;then
    null
    ;else
    ;Each row in tt is an environment
    ;We can use bool-eval to evaluate the expression on 1 environment
    (cons ;3.) Combine the results
     ;1.) Evaluate on the first row
     (bool-eval expression (first tt))
     ;2.) Evaluate on the rest
     (run-on-truth-table expression (rest tt))
    );End of cons
  );end of if
)
;Tests
...
(define (atleast-one-true my_list)
  ;base case the list is empty return t
  (if (null? my_list)
    ;then
    #f
    ;else
      (or (first my_list) 
          (atleast-one-true (rest my_list)))
  );end if
)
;Tests
...
(define (is-satisfied? bool-expression)
  ;If running the expression on it's truth table returns atleast one true, 
  ;then it is satisfied
  
  ;We need to make the truth table for the expression first 
  ;by getting all it's variables.
  (atleast-one-true
   (run-on-truth-table bool-expression
                       (make-truth-table (get-variables bool-expression))
   );end run-on-truth-table
  );end atleast-one-true
)
;Test Case
